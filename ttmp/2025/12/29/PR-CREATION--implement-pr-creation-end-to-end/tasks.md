...
- [ ] Implement error handling with PR data save: On PR creation failure save generated PR data to file (e.g., .pr-builder/pr-data-<timestamp>.yaml), include clear error message indicating where data was saved, exit with appropriate error code
- [ ] Implement base branch handling (--base): Add --base flag to create command, default to main (or detected default branch via git.GetDefaultBranch()), pass --base flag to gh pr create
...
- [ ] Add tests for PR creation: Create prescribe/internal/github/github_test.go, test gh pr create command construction with various flags, mock gh command execution for unit tests
